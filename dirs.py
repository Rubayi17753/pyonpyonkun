ids_data_fp = 'data/raw/babelstone/IDS.TXT'
ids_processed_fp = 'data/intermediate/ids_preprocessed.tsv'
ids_comments_fp = 'data/intermediate/ids_comments.tsv'
ids_elements_fp_json = 'data/intermediate/ids_elements.json'
ids_elements_fp_json_lite1 = 'data/intermediate/ids_elements_lite1.json'
ids_elements_fp_json_lite2 = 'data/intermediate/ids_elements_lite2.json'
ids_elements_fp_tsv = 'data/intermediate/ids_elements.tsv'

ids_decomposed_fp = 'data/output/ids_decomposed.tsv'
dict_yaml_fp = 'data/output/pyonpyonkun.dict.yaml'

freqlist_fp = 'data/raw/freqs/composite.tsv'
strokelist_fp = 'data/raw/complete-strokes.txt'

assigned_fp = 'config/elements.yaml'
checklist_fp = 'config/uncategorized.tsv'
secondaries_json_fp = 'config/secondaries.json'
secondaries_csv_fp = 'config/secondaries.tsv'

ids_lint_report_fp = 'data/output/ids_lint.txt'